...
from utils import json_keys, settings, utils
...
def dump_os_domains(domain_os_flows, fpath):
    ...
    for fn in os.listdir(fpath):
        ...
        with open(full_path) as jf:
            ...
            for k in data:
                entry = data[k]
                host = entry.get(json_keys.host, None)

                # Account for undefined domains
                domain = entry.get(json_keys.domain, None)
                if domain is None or len(domain) == 0:
                    if host is None:
                        # ????
                        domain = entry.get(json_keys.domain, None)
                        entry[json_keys.domain] = domain
                    else:
