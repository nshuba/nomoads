...
/**
 * Trains on everything: URL + headers + apps + pii
 */
class UrlHeadersAppsPiiAdsTrainer extends UrlHeadersAppsAdsTrainer {

    public UrlHeadersAppsPiiAdsTrainer(ServerUtils serverUtils) {
        super(serverUtils);
    }

    @Override
    public Instances populateArff(Info info, TrainingData trData, int theta) {
        System.out.println("pop arff: " + UrlHeadersAppsPiiAdsTrainer.class.getSimpleName());
        ...
        while (all.hasNext()) {
            ...
            for (Object piiObj : trData.piiLabels.get(count)) {
                instanceValue[fi.get(piiObj)] = 1; // Set as 1 when PII is present
            }
    ...
    protected Instance convertObjectToInstance(JSONObject packet, AdsPredictor predictor, String
            domainOS) {
        System.out.println("convertObjectToInstance: " + UrlHeadersAppsPiiAdsTrainer.class
                .getSimpleName());
